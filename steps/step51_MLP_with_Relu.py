...
if __name__ == "__main__":
    ...
    plt.subplot(1, 2, 1)
    plt.title("MNIST-train/test loss")
    plt.plot(history["train_loss"], label="train loss")
    plt.plot(history["test_loss"], label="test loss")
    plt.legend(loc="best")
    
    plt.subplot(1, 2, 2)
    plt.title("MNIST-train/test accuracy")
    plt.plot(history["train_accuracy"], label="train accuracy")
    plt.plot(history["test_accuracy"], label="test accuracy")
    plt.legend(loc="best")
    
    plt.savefig("./viz_images/step51(Relu_Adam)_MNIST_train_test_loss_acc.png")
    plt.show()

    """
    SGD Optimizer:
    train loss: 0.2754, accuracy: 0.9222
    test loss: 0.2562, accuracy: 0.9298
    
    Adam Optimizer:
    train loss: 0.0272, accuracy: 0.9915
    test loss: 0.1021, accuracy: 0.9718
    """